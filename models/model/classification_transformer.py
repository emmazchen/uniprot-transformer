...
class TorchTransformerControl(nn.Module):
    def __init__(self, mconfigs):
        super().__init__()
        self.pad_idx = mconfigs['pad_idx']
        self.emb = Embedding(mconfigs['encoder']['src_vocab_size'], mconfigs['encoder']['d_embedding'], padding_idx=0)
...
class MLPControl(nn.Module):
    def __init__(self, mconfigs):
        super().__init__()
        self.pad_idx = mconfigs['pad_idx']
        self.embed = EmbeddingBlock(d_embedding=60, vocab_size=mconfigs['encoder']['src_vocab_size'], max_seq_len=mconfigs['encoder']['max_seq_len'],p_drop=.1)
        self.avg_pool_reduce_dim = torch.mean # average along dimension to get (batch_size, d_embedding)
        self.mlp = nn.Sequential(
            nn.Linear(mconfigs['encoder']['d_embedding'],mconfigs['mlp']['l1']['out_features']),
            nn.ReLU(),
